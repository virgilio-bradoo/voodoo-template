...
class IncomingShipment(report_sxw.rml_parse):
...
#            self.cr, self.uid, invoice_address_id)
    ...
    def __init__(self, cr, uid, name, context):
        super(IncomingShipment, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
#            'invoice_address': self._get_invoice_address,
        })

report_sxw.report_sxw('report.webkit.delivery_slip_in',
                      'stock.picking',
                      'addons/stock_picking_report_custom/incoming_shipment_order.mako',
                      parser=IncomingShipment)
